...
class User{
    ...
    public static function GetUser($id){
        $sqlStatement = "Select * from customer where id = '$id'";
        $result = User::$conn->query($sqlStatement);
        if ($result->num_rows == 1) {
            $userData = $result->fetch_assoc();
            return new User($userData['id'], $userData['name'], $userData['surname'], $userData['email'], $userData['password']);
        }
    ...
    public static function GetUserInfo($id){
        $sqlStatement = "Select id, name, email, surname from Users where id=$id";
        $result = User::$conn->query($sqlStatement);
        if ($result->num_rows > 0) {
    ...
    private function __construct($newId, $newName, $newMail, $newSurname, $password){
        $this->id = $newId;
        $this->name = $newName;
        $this->email = $newMail;
